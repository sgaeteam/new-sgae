$(document).ready(function() {
	...
	var loadDataTable = function(){
		...
		if(user != '') { 
			$.ajax({
				...
				url: 'perfil_exec.php?act=select',
					...
					if (s !== null) {
						oTable.clear().draw();
					 	for(var i = 0; i < s.length; i++) {
					 		oTable.row.add([s[i][0],s[i][1],s[i][2],s[i][3],"<a class='btn default btn-xs green-stripe' style='padding: 1px 5px; display: inline;' href=perfil_view.php?idusu="+s[i][4]+"><span class='glyphicon glyphicon-eye-open'></span> Exibir</a>"+
												"<a class='btn default btn-xs yellow-stripe' style='padding: 1px 5px; display: inline;' href=perfil_edit.php?idusu="+s[i][4]+"><span class='glyphicon glyphicon-pencil'></span> Alterar</a>"+
					                            ...
					                            "               Você deseja excluir o perfil <strong>"+s[i][0]+"</strong>? Esta operação não poderá ser desfeita."+
					                            "            </div>"+
					                            "            <div class='modal-footer'>"+
					                            "                <button type='button' class='btn btn-warning' data-dismiss='modal'>Cancelar</button>"+
					                            "                <a href=perfil_exec.php?act=delete&idusu="+s[i][4]+" id='modalDelete' type='button' class='btn btn-danger'>Excluir</a>"+
					                            "            </div>"+
					                            "        </div>"+
					                            "    </div>"+
					                            "</div>"]).draw(false);
				...
				complete: function(){
					oTable.page(parseInt(localStorage.getItem('perfil_search_ajax_table_page'),10)).draw( 'page' );
				    $.unblockUI();
				},
				error: function(e) {
	...
	oTable.on('page.dt', function() { 
		localStorage.setItem('perfil_search_ajax_table_page', oTable.page.info().page);
	});
	...
	if (loadCriteria == 1) {
		...
		localStorage.setItem('perfil_search_ajax_table_page', 0);
	}
